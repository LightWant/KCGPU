...
__device__ void build_induced_subgraph_noOri(
    ...
    )
{
    ...
    for (T j = wx; j < srcLen; j += numPartitions)
    {
        ...
        // }
        // __syncwarp(partMask);
        graph::warp_sorted_count_and_encode_full<BLOCK_SIZE/CPARTSIZE, T, true, CPARTSIZE>(
...
__global__ void
kckernel_edge_block_warp_binary_count_lc(
	uint64* counter,
...
)
{
	...
	for (unsigned long long i = blockIdx.x; i < (unsigned long long)current.count[0]; i += gridDim.x)
	{
        ...
        if(threadIdx.x == 0) level_offset = sm_id * CBPSM * (numPartitions * NUMDIVS * MAXDEPTH) + levelPtr * (numPartitions * NUMDIVS * MAXDEPTH);
		__syncthreads();
...
__launch_bounds__(BLOCK_DIM_X, 16)
__global__ void
kckernel_node_block_warp_binary_count_lc(
    uint64* counter,
    graph::COOCSRGraph_d<T> g,
    const  graph::GraphQueue_d<T, bool>  current,
